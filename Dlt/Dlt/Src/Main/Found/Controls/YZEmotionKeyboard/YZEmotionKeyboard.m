...
- (void)setupBottonButton
{
    ...
    NSString *imageN = [NSString stringWithFormat:@"%@/smile",[YZEmotionManager emotionBundleName]];
...
//    }
  [self requestSendMessageContent];
}

- (void)requestSendMessageContent{
  if (self.delegate && [self.delegate respondsToSelector:@selector(emotionKeyboard:didClickedSendEmotionDescribeMessages:)]) {
    [self.delegate emotionKeyboard:self didClickedSendEmotionDescribeMessages:[self emotionText]];
  }
}
...
- (NSString *)emotionText
{
    ...
    [_yz_textView.attributedText enumerateAttributesInRange:NSMakeRange(0, _yz_textView.attributedText.length) options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired
