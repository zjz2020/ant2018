...
{
    return [self ba_requestWithType:BAHttpRequestTypePost urlString:urlString parameters:parameters successBlock:successBlock failureBlock:failureBlock progress:progress];
}
    ...
    else if (type == BAHttpRequestTypePost)
    {
        sessionTask = [[self sharedAFManager] POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
            ...
            if ([responseObject valueForKey:@"code"]) {
                NSString *str = [NSString stringWithFormat:@"%@",[responseObject valueForKey:@"code"]];
                
                if ([str isEqualToString:@"-1"]) {
                    DLTUserProfile * user = [DLTUserCenter userCenter].curUser;
                    NSString *passWord = [SAMKeychain passwordForService:DLTKeychainserviceNameKey account:user.phone];
                    [[[DLTUserCenter userCenter] tokenLogin:user.phone
                                                   password:passWord]
        ...
        [imageArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            {   // 图片数据不为空才传递 fileName
                ...
                [formData appendPartWithFileData:imgData
                                            name:[NSString stringWithFormat:@"picflie%d_%@",(int)idx,[NSUUID UUID].UUIDString]
                                        fileName:@"image.png"
                                        mimeType:@" image/jpeg"];
              /*
                [formData appendPartWithFileData:imgData
